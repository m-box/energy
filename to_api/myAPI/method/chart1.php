...
{$err_n=0;
	...
	$where=implode(' AND ', $w);
	$rows = $db -> getAll("SELECT * FROM `data` WHERE `id_object`=?s AND `type`=?s AND ?p" , $data['id'] , $data['param'] , $where);
	//print_r($db -> parse ("SELECT * FROM `data` WHERE `id_object`=?s AND `type`=?s AND ?p" , $data['id'] , $data['param'] , $where));
	for ($i=0;$i<count($rows);$i++)
	{$row[$i]=array($rows[$i]['date'], $rows[$i]['val']);}
	$cols= array('Дата'=>'string', 'Показник'=>'number');
	    $dataTable = array(
    'cols' => $cols,
    'rows' => $row
    );
	//$dataTable['parse']=$db -> parse ("SELECT * FROM `data` WHERE `id_object`=?s AND `type`=?s AND ?p" , $data['id'] , $data['param'] , $where);
	return $dataTable;
}
?>