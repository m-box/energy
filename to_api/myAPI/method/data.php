...
if ($us['id_user_type']==3)
{
	...
	if (!isset($data['date']) or $data['date']=='')
		...
		{
		$obj = $db -> getAll("SELECT * FROM `data` WHERE `id_object`=?s AND `type`=?s AND `date`='?p'" , $data['id_object'] , $data['type'] , $data['date']); 
		if (count($obj)==0)
			{
				$db -> query("INSERT INTO `data` (
				`id` ,
				...
				" , $data['id_object'] , $data['date'] , $data['type'] , $data['val']); return 'ok';
			}
		else
			{
				$id=$obj[0]['id'];
